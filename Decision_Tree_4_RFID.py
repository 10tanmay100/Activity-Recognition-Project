...
lg=logging_file.define_logger("logger_4_RFID_Decision_Tree")
lg.basic_config()
#reading the dataset
try:
...
# models=[]
# for cc in path["ccp_alphas"]:
#     dt=DecisionTreeClassifier(ccp_alpha=cc)
...
# for cc in path['ccp_alphas']:
#     if (cc>=0.0) & (cc<=0.05):
#         selected_ccps.append(cc)
# print(selected_ccps)




# plt.plot(path["ccp_alphas"],train_score,marker="o",label="trained")
# plt.plot(path["ccp_alphas"],test_score,marker="o",label="tested")
# plt.xlabel("CCP")
# plt.ylabel("Accuracy")
# plt.legend(loc="best")
# plt.show()
...
try:
    dt=DecisionTreeClassifier(criterion='gini', max_depth=9,max_features='log2',min_samples_leaf=8,min_samples_split=3 ,splitter= "random",ccp_alpha=0.01)
...
try:
    ...
    lg.error(f"Error while dt fitting!! and error is {e}")
#predict the data
try:
    y_train_pred=dt.predict(x_train)
    lg.info("Prediction trained data!!")
except Exception as e:
    lg.error("Error while doing prediction on train data")
print(classification_report(y_train,y_train_pred))

try:
    y_test_pred=dt.predict(x_test)
    lg.info("Prediction tested data!!")
except Exception as e:
    lg.error("Error while doing prediction on test data")
print(classification_report(y_test,y_test_pred))

# try:
#     g_dt=GridSearchCV(estimator=dt,param_grid=param,cv=50,verbose=True)
#     lg.info("Grid Search CV Has been applied")
# except Exception as e:
#     lg.error(f"Error has occured while grid search cv error is {e}")
# try:
#     g_dt.fit(x_train,y_train)
#     lg.info("Fitting has been done on Grid Search CV")
# except Exception as e:
#     lg.error(f"Error has happened while fitting in grid search cv error is {e}")
#     print(e)
...
# lg.info("Applying Decison Tree classifier")
# try:
#     dt=DecisionTreeClassifier(criterion='log_loss', max_depth=4, min_samples_split=8, splitter= "random",ccp_alpha=0.01)
#     lg.info("DecisionTreeClassifier object created!!")
# except Exception as e:
#     lg.error("Error has happened while creating object!!")
# #fit the data
