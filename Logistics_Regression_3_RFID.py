...
lg=logging_file.define_logger("logger_3_RFID_Logistics")
lg.basic_config()
#reading the dataset
try:
...
try:
    lr=LogisticRegression(penalty='l2',tol=0.1,solver="newton-cg",multi_class="multinomial",C=0.1)
except Exception as e:
...
try:
    ...
    lg.info("prediction done on trained data")
except Exception as e:
    lg.error(f"Error has happended while prediction on training data error is {e}")
y_train_pred_prob=lr.predict_proba(x_test)
...
try:
    ...
    lg.info("Prediction done on test data")
except Exception as e:
    lg.error(f"Error has happended while doing prediction on test data")
...
# try:
#     output=g.best_params_
#     lg.info("Best Parameters has been found on Grid Search CV")
#     print(output)
# except Exception as e:
