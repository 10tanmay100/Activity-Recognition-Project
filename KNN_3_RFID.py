...
lg=logging_file.define_logger("logger_3_RFID_knn")
lg.basic_config()
#reading the dataset
try:
...
lg.info("Creating Logistics Regression Model")
try:
    knn=KNeighborsClassifier(n_neighbors=3,weights="distance",algorithm="kd_tree",leaf_size=10,p=2)
except Exception as e:
    lg.error("Error has happened on the line 272 while creating KNN model!!")
try:
    lg.info("fitting the model")
...
try:
    ...
    lg.error(f"error has happened while predict test data, error is {e}")

lg.info("Plotting classification report for training")
...
lg.info("Plotting classificatio report for testing")
print(classification_report(y_test,y_test_pred))
