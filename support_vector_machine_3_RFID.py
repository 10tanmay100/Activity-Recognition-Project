...
lg=logging_file.define_logger("logger_3_RFID_svm")
lg.basic_config()
#reading the dataset
try:
...
try:
    svm=SVC(C=1,kernel="poly",degree=3,tol=1e-3,gamma="scale",max_iter=5,decision_function_shape="ovr")
except Exception as e:
    lg.error(f"Error has happened on the line 272 while creating SVC model!! error is {e}")
try:
...
try:
    ...
    lg.info("Prediction on train data")
except Exception as e:
    lg.error(f"error while training data and the errors")

lg.info("Plotting classification report on train data")
...
#     "gamma":['scale', 'auto'],
#     "p":[1,2],
#     "tol":[1e-3,1e-2,1e-1,1e-4,1e-5,1e-6],
#     "max_iter":range(0,50,1)
# }
# g_svm=GridSearchCV(estimator=svm,param_grid=param,verbose=True)
# g_svm.fit(x_train,y_train)

# print(g_svm.best_params_)
try:
    y_test_pred=svm.predict(x_test)
    y_test_pred_prob=svm.predict_proba(x_test)
    lg.info("Prediction on test data")
except Exception as e:
    lg.error(f"error while testing data and the errors")
