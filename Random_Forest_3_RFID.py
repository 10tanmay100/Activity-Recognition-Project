...
lg=logging_file.define_logger("logger_3_RFID_Random_forest")
lg.basic_config()
#reading the dataset
try:
...
lg.info("Applying RandomForestClassifier on train data")
try:
...
try:
    ...
    lg.error("Error while RANDOM_FOREST fitting!!")
#predict the data
...
lg.info("Applying RandomForestClassifier on test data")
...
try:
    ...
    lg.error(f"Error while prediction on test error is {e}")
print(classification_report(y_test,y_test_pred))
import pickle

pickle.dump(r,open("3_RFID.pkl","wb"))

pickle.dump(st,open("3_RFID_Scale.pkl","wb"))
